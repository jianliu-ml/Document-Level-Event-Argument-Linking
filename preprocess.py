...
def _get_all_trigger(data):
    ...
    for elem in data:
        ...
        s, e, t = trigger
        # print(trigger)
        trigger_word = ' '.join(long_sentence[s:e+1])
        result.setdefault(t, set())
        result[t].add(trigger_word)
    return result
...
if __name__ == '__main__':
    ...
    # for elem in read_corpus('data/test.jsonlines'):
    #     _, sentences, trigger, _, arguments = elem
    #     long_sentence = [item for sublist in sentences for item in sublist]

    #     z = {}
    #     z[trigger[0]] = trigger[-1]

    #     for arg in arguments:
    #         z[arg[0]] = arg[-1]

    #     for i, t in enumerate(long_sentence):
    #         m = z[i] if i in z else ''
    #         print(t, m)

    # result = _get_all_trigger(train_data)
    # print(result)